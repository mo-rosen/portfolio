...
public class OregonTrailUtil {
    ...
    public static void quarantineCare(Traveler[] passengerArray, int request) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        for (Traveler person : passengerArray) {
            ...
            if (person instanceof Ranger) {
                boolean foraged = rangerTry(Ranger.class.getMethod("forageMedicine", Traveler.class), getIsHealthy, passengerArray, person);
                boolean healed = rangerTry(Ranger.class.getMethod("cook", Traveler.class, int.class), getIsHealthy, passengerArray, person);
                if(foraged || healed) {
                    break;
                }
            }
    ...
    public static boolean rangerTry(Method specialMethod, Method isHealthy, Traveler[] passengerArray, Traveler r) throws InvocationTargetException, IllegalAccessException {
        for (Traveler fellowTraveler : passengerArray) {
            if (!((boolean) isHealthy.invoke(fellowTraveler))) {
                specialMethod.invoke(r, fellowTraveler);
                return true;
            }
        }
        return false;
